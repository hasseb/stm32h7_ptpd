...
{
  // Initialize the main PTP data structure.
  ...
  // Initialize the foreign records buffers.
  ...
  for (;;)
  {
    ...
    if (!netif_is_up(&gnetif) || ip4_addr_isany_val(gnetif.ip_addr))
    {
      // Wait until the network interface comes up.
      while (!netif_is_up(&gnetif) || ip4_addr_isany_val(gnetif.ip_addr)) sys_msleep(500);
...
void ptpd_init(bool slave_only)
{
  ...
  if (sys_mbox_new(&ptp_alert_queue, 8) == ERR_OK)
  {
    ...
    t = sys_thread_new("PTPD", ptpd_thread, NULL, DEFAULT_THREAD_STACKSIZE, osPriorityHigh);
...
// Get the pointer to the static PtpClock instance
PtpClock* ptpd_get_instance(void)
{
	return &ptp_clock;
}

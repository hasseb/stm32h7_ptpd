...
void ptpd_servo_update_clock(PtpClock *ptp_clock)
{
  ...
  if (ptp_clock->currentDS.offsetFromMaster.seconds != 0 || abs(ptp_clock->currentDS.offsetFromMaster.nanoseconds) > MAX_ADJ_OFFSET_NS)
  {
    ...
    if (!ptp_clock->servo.noAdjust)
    {
      ...
      {
        ptp_clock->adjust = ptp_clock->currentDS.offsetFromMaster.nanoseconds > 0 ? ADJ_FREQ_MAX : -ADJ_FREQ_MAX;
        ptpd_adj_freq(-ptp_clock->adjust);
      }
    }
  }
  else
  {
    ...
    if (!ptp_clock->servo.noAdjust)
    {
      ptp_clock->adjust = offsetNorm / ptp_clock->servo.ap + ptp_clock->observedDrift;
      ptpd_adj_freq(-ptp_clock->adjust);
    }

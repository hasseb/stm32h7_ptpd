...
char *ptpd_state_string(uint8_t state)
{
  switch (state)
  {
...
{
  DBG("leaving state %s\n", ptpd_state_string(ptp_clock->portDS.portState));
  ...
  switch (ptp_clock->portDS.portState)
  {
    ...
    case PTP_PASSIVE:
      ...
      if (get_flag(ptp_clock->events, STATE_DECISION_EVENT))
      {
        ...
        DBGV("recommending state %s\n", ptpd_state_string(ptp_clock->recommendedState));
...
static void issue_sync(PtpClock *ptp_clock)
{
  ...
  else
  {
    ...
    {
    	ERROR("issue_sync: can't sent follow up\n");
    }
  }
}
